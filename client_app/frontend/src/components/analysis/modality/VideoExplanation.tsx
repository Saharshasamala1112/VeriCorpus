import { useState } from 'react'
import { Badge } from '../../ui'
import type { ExplanationResult } from '../../../types/explainability'
import { getExplanationTemplate } from '../../../i18n'

interface VideoExplanationProps {
  explanation: ExplanationResult
  language: string
}

export default function VideoExplanation({ explanation, language }: VideoExplanationProps) {
  const template = getExplanationTemplate(language)
  const [selectedFrame, setSelectedFrame] = useState<number | null>(null)
  const [showTimeline, setShowTimeline] = useState(true)

  const videoEvidence = explanation.evidence.filter((e) =>
    ['suspicious_frame', 'temporal_segment'].includes(e.type),
  )

  if (videoEvidence.length === 0) {
    return <p className="text-xs text-slate-500 italic">{template.noExplanationAvailable}</p>
  }

  const suspiciousFrames = videoEvidence.filter((e) => e.type === 'suspicious_frame')
  const temporalSegments = videoEvidence.filter((e) => e.type === 'temporal_segment')

  // Calculate total duration for timeline
  const maxEndTime = Math.max(
    ...temporalSegments.map(
      (seg) =>
        ((seg.metadata?.startTime as number) ?? 0) + ((seg.metadata?.duration as number) ?? 0),
    ),
    ...suspiciousFrames.map((f) => (f.metadata?.timestamp as number) ?? 0),
    0,
  )

  return (
    <div className="space-y-4">
      {/* Video Timeline */}
      <div className="rounded-lg border border-amber-500/20 bg-amber-500/5 p-3">
        <div className="flex items-center justify-between">
          <Badge variant="warning" size="sm">
            Video Timeline
          </Badge>
          <button
            onClick={() => setShowTimeline(!showTimeline)}
            className="rounded bg-slate-800 px-2 py-1 text-[10px] text-slate-400 hover:bg-slate-700"
          >
            {showTimeline ? 'Hide' : 'Show'} Timeline
          </button>
        </div>

        {showTimeline && maxEndTime > 0 && (
          <div className="mt-3 overflow-hidden rounded-lg border border-slate-700/50 bg-slate-900 p-2">
            {/* Timeline visualization */}
            <div className="relative h-16">
              {/* Background bar */}
              <div className="absolute top-1/2 h-1 w-full -translate-y-1/2 rounded-full bg-slate-700" />

              {/* Temporal segments */}
              {temporalSegments.map((seg, i) => {
                const startTime = (seg.metadata?.startTime as number) ?? 0
                const duration = (seg.metadata?.duration as number) ?? 0
                const left = (startTime / maxEndTime) * 100
                const width = (duration / maxEndTime) * 100
                return (
                  <div
                    key={`seg-${i}`}
                    className="absolute top-1/2 h-3 -translate-y-1/2 rounded bg-cyan-500/40"
                    style={{ left: `${left}%`, width: `${width}%` }}
                    title={`Segment: ${startTime.toFixed(1)}s - ${(startTime + duration).toFixed(1)}s`}
                  />
                )
              })}

              {/* Suspicious frame markers */}
              {suspiciousFrames.map((frame, i) => {
                const timestamp = (frame.metadata?.timestamp as number) ?? 0
                const left = (timestamp / maxEndTime) * 100
                const isSelected = selectedFrame === i
                return (
                  <button
                    key={`marker-${i}`}
                    onClick={() => setSelectedFrame(selectedFrame === i ? null : i)}
                    className={`absolute top-0 h-full w-2 -translate-x-1/2 rounded ${
                      isSelected ? 'bg-rose-500' : 'bg-amber-500'
                    } hover:bg-rose-400`}
                    style={{ left: `${left}%` }}
                    title={`Suspicious frame at ${timestamp.toFixed(2)}s`}
                  />
                )
              })}

              {/* Time axis */}
              <div className="absolute -bottom-4 left-0 right-0 flex justify-between text-[8px] text-slate-600">
                <span>0:00</span>
                <span>{`0:${maxEndTime.toFixed(0).padStart(2, '0')}`}</span>
              </div>
            </div>

            {/* Legend */}
            <div className="mt-4 flex items-center gap-4 text-[10px] text-slate-500">
              <div className="flex items-center gap-1">
                <div className="h-2 w-2 rounded-full bg-cyan-500/40" />
                <span>Temporal Segment</span>
              </div>
              <div className="flex items-center gap-1">
                <div className="h-2 w-2 rounded-full bg-amber-500" />
                <span>Suspicious Frame</span>
              </div>
            </div>
          </div>
        )}

        {/* Stats */}
        <div className="mt-2 flex gap-4 text-[11px] text-slate-500">
          <span>{suspiciousFrames.length} suspicious frame(s)</span>
          <span>{temporalSegments.length} temporal segment(s)</span>
          <span>Total duration: {maxEndTime.toFixed(1)}s</span>
        </div>
      </div>

      {/* Suspicious Frames */}
      {suspiciousFrames.length > 0 && (
        <div className="rounded-lg border border-amber-500/20 bg-amber-500/5 p-3">
          <div className="flex items-center justify-between">
            <Badge variant="warning" size="sm">
              {template.video.suspiciousFramesLabel} ({suspiciousFrames.length})
            </Badge>
            <span className="text-[11px] text-slate-500">
              {suspiciousFrames.length} frame(s) flagged
            </span>
          </div>

          {/* Frame Selection Buttons */}
          <div className="mt-2 flex flex-wrap gap-1">
            {suspiciousFrames.map((_, i) => (
              <button
                key={i}
                onClick={() => setSelectedFrame(selectedFrame === i ? null : i)}
                className={`rounded px-2 py-1 text-[10px] ${
                  selectedFrame === i
                    ? 'bg-amber-500/30 text-amber-300'
                    : 'bg-slate-800 text-slate-400 hover:bg-slate-700'
                }`}
              >
                Frame {i + 1}
              </button>
            ))}
          </div>

          {/* Frame Details */}
          <div className="mt-2 space-y-2">
            {suspiciousFrames.map((ev, i) => (
              <div
                key={`frame-${i}`}
                className={`rounded p-2 ${
                  selectedFrame === i ? 'bg-amber-500/10' : 'bg-slate-950/30'
                }`}
              >
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    <span className="text-[11px] text-slate-400">Frame {i + 1}</span>
                    {!!ev.metadata?.faceSwapDetected && (
                      <Badge variant="danger" size="sm">
                        Face swap
                      </Badge>
                    )}
                  </div>
                  <span className="text-[10px] text-slate-500">
                    {(ev.relevance * 100).toFixed(0)}% confidence
                  </span>
                </div>
                <p className="mt-1 text-xs text-slate-400">{ev.description}</p>
                <div className="mt-1 flex flex-wrap gap-3 text-[10px] text-slate-500">
                  {ev.metadata?.frameIndex != null && (
                    <span>Frame: #{ev.metadata.frameIndex as number}</span>
                  )}
                  {ev.metadata?.timestamp != null && (
                    <span>Time: {(ev.metadata.timestamp as number).toFixed(2)}s</span>
                  )}
                </div>
              </div>
            ))}
          </div>
        </div>
      )}

      {/* Temporal Segments */}
      {temporalSegments.length > 0 && (
        <div className="rounded-lg border border-cyan-500/20 bg-cyan-500/5 p-3">
          <div className="flex items-center justify-between">
            <Badge variant="info" size="sm">
              {template.video.temporalSegmentsLabel} ({temporalSegments.length})
            </Badge>
          </div>
          <div className="mt-2 space-y-2">
            {temporalSegments.map((ev, i) => (
              <div key={`seg-${i}`} className="rounded bg-slate-950/30 p-2">
                <div className="flex items-center justify-between">
                  <span className="text-[11px] text-slate-400">Segment {i + 1}</span>
                  <span className="text-[10px] text-slate-500">
                    {(ev.relevance * 100).toFixed(0)}%
                  </span>
                </div>
                <p className="mt-1 text-xs text-slate-400">{ev.description}</p>
                <div className="mt-1 flex gap-4 text-[10px] text-slate-500">
                  {ev.metadata?.startTime != null && (
                    <span>Start: {(ev.metadata.startTime as number).toFixed(1)}s</span>
                  )}
                  {ev.metadata?.duration != null && (
                    <span>Duration: {(ev.metadata.duration as number).toFixed(1)}s</span>
                  )}
                </div>
              </div>
            ))}
          </div>
        </div>
      )}
    </div>
  )
}
